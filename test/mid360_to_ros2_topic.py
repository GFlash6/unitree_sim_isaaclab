...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Forward Isaac MID360 shared-memory points to a ROS2 PointCloud2 topic.")
    parser.add_argument("--topic", default="/mid360/points", help="ROS2 PointCloud2 topic.")
    parser.add_argument("--frame-id", default="mid360_link", help="PointCloud2 frame_id. Use RViz Fixed Frame = this value.")
    parser.add_argument("--rate", type=positive_float, default=10.0, help="Publish rate in Hz.")
    parser.add_argument("--max-points", type=positive_int, default=20000, help="Publish at most this many points.")
    parser.add_argument("--once", action="store_true", help="Publish one cloud and exit.")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        raise SystemExit("rclpy is not importable. Run this after sourcing your ROS2 environment.") from exc
    ...
    def publish() -> None:
        ...
        if points.size == 0:
            return
        msg = cloud_msg(points, args.frame_id, timestamp_msg(reader.last_timestamp_ns))
        publisher.publish(msg)
        now = time.monotonic()
        if now - last_log > 1.0:
            node.get_logger().info(
                f"published fresh frame seq={reader.last_sequence} "
                f"stamp_ns={reader.last_timestamp_ns} points={points.shape[0]} "
                f"on {args.topic}"
            )
            last_log = now
        published_once = True
